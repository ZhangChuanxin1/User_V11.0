...
public class Login<ener> extends Activity {                 //登录界面活动
//成员变量
    ...
    private TextView mChangepwdTextgepwdText;
    private UserDataManager mUserDataManager;         //用户数据管理类

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);//保存当前状态 比如记住密码
        ...
        login_sp = getSharedPreferences("userInfo", 0);//两个参数 第一个参数是存储时的名称，第二个参数则是文件的打开方式~
    ...
    public void login() {                                              //登录按钮监听事件
        if (isUserNameAndPwdValid()) {
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确
                ...
                Intent intent = new Intent(Login.this,Camera1.class) ;    //切换Login Activity至User Activity
                startActivity(intent);
                //finish();
                Toast.makeText(this, " 请进行人脸验证 ",Toast.LENGTH_SHORT).show();//登录成功提示
            }else if(result==0){
